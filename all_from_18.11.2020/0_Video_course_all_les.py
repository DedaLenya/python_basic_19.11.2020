...
Подсчёт одинаковых слов #124
В этом уроке нужно написать программу, которая получает строку от пользователя с помощью input.
Программа должна разделить строку на слова по символу пробела, точке или запятой и подсчитать,
сколько раз каждое из слов встречается в строке. Разбить на слова будет проще, если предварительно
заменить точки и запятые на символы пробела. Слова должны считаться одинаковыми без учёта регистра.
То есть «Питон» и «питон» — одно и тоже слово. Пустые слова, которые могут появится из‑за двух
и более пробелов подряд, не должны участвовать в подсчёте.
...
не : 1
выпрограммировали : 1
В этом уроке не будет конкретных указаний для выполнения. Думать придется самостоятельно. Однако,
можно с уверенностью сказать, что знания о словарях вам пригодятся.

some_dict = {}
some_string = ("программу,Которая.получает строку строку").lower()
some_string = some_string.replace(",", " ")
some_string = some_string.replace(".", " ")
some_string = some_string.split()

for words in some_string:
...
user_input = ("программу,программу которая аделает тоже,программу тоже самое")
...
sort_key = dict.keys(words)
sort_key = sorted(sort_key)
print(type(sort_key))
print(sort_key)
for word in sort_key:
